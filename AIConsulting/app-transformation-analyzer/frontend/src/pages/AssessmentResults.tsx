...
const AssessmentResultsPage: React.FC = () => {
  ...
  const [mockAssessment, setMockAssessment] = useState<TwelveFactorAssessment | null>(null);
  const [anchorEl, setAnchorEl] = useState<null | HTMLElement>(null);
  const [exportLoading, setExportLoading] = useState(false);

  // Load mock assessment data from API
  useEffect(() => {
    const loadAssessment = async () => {
      setLoading({ loading: true });
      try {
        const response = await fetch('http://localhost:5001/api/assessment/mock');
        const data = await response.json();

        if (data.success) {
          setMockAssessment(data.data.assessment);
          setLoading({ loading: false });
        } else {
          setLoading({ loading: false, error: 'Failed to load assessment data' });
        }
      } catch (error: any) {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      // Create a simple mock summary for PDF export
      const summary = {
        overview: {
          grade: mockAssessment.grade,
          overall_score: mockAssessment.overall_score,
          weighted_score: mockAssessment.weighted_score,
          confidence: mockAssessment.confidence,
          coverage: mockAssessment.coverage
        },
      ...
      await exportAssessmentToPDF(mockAssessment, summary as any, {
        includeCharts: true,
  ...
  const getRadarChartData = () => {
    ...
    return Object.entries(mockAssessment.factor_evaluations).map(([name, evaluation]) => ({
      factor: name,
  ...
  const getHeatmapData = () => {
    ...
    return mockAssessment.gaps.map(gap => ({
    ...
    <Box>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            Interactive analysis of {mockAssessment.repository_url}
          </Typography>
        </Box>
      ...
      <Grid container spacing={3} mb={4}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              <Typography variant="h3" color="primary" gutterBottom>
                {Math.round(mockAssessment.overall_score * 20)}%
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Compliance Score
              </Typography>
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              <Typography variant="h3" color="success.main" gutterBottom>
                {mockAssessment.summary.excellent_factors}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Excellent Factors
              </Typography>
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              <Typography variant="h3" color="error.main" gutterBottom>
                {mockAssessment.summary.critical_issues}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Critical Issues
              </Typography>
      ...
      <Paper sx={{ width: '100%' }}>
            ...
            <Grid item xs={12} lg={4}>
              <Card>
                <CardContent>
                  ...
                  <Box mb={2}>
                    <Typography variant="body2" color="text.secondary">
                      Repository: {mockAssessment.repository_url}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Date: {new Date(mockAssessment.timestamp).toLocaleDateString()}
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      Confidence: {Math.round(mockAssessment.confidence * 100)}%
                    </Typography>
                  </Box>
                  <Divider sx={{ my: 2 }} />
                  ...
                  <Typography variant="body2" color="text.secondary">
                    • {mockAssessment.summary.total_factors} factors evaluated
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    • {mockAssessment.summary.total_recommendations} recommendations
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    • {mockAssessment.summary.total_gaps} gaps identified
                  </Typography>
                </CardContent>
              </Card>
            </Grid>
        ...
        <TabPanel value={tabValue} index={1}>
          {/* Factor Analysis Tab */}
          <FactorCards
            factors={mockAssessment.factor_evaluations}
            onFactorClick={handleFactorClick}
            showEvidence={true}
            groupBy="score"
          />
            ...
            <Grid item xs={12} md={6}>
              <Card>
                <CardContent>
                  ...
                  {Object.entries(mockAssessment.score_distribution || {}).map(([scoreName, count]) => {
                    const total = mockAssessment.summary.total_factors;
